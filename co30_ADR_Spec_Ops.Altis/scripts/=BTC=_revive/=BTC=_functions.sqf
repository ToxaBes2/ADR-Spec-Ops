...
BTC_fnc_wait_for_revive =
{
	...
	_timeout = serverTime;
	BTC_r_timeout = _timeout + BTC_revive_time_max;
	private ["_id","_lifes"];
...
BTC_3d_markers =
{
	...
	{
		{
		    _showAll = partizan_ammo getVariable ['DIPLOMACY', 0];
			if (((_x distance player) < BTC_3d_distance) && ((playerSide == west && _x isKindOf 'B_Soldier_base_F') || (playerSide == resistance && _x isKindOf 'I_G_Soldier_base_F') || _showAll == 1) && (format ["%1", _x getVariable "BTC_need_revive"] == "1") && (player != _x)) then
			{
