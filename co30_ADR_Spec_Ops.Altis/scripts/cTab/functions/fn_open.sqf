...
if (_vehicle != _player && (_isDialog || _displayName in ["cTab_TAD_dsp"])) then {
	cTabIfOpen set [5,
		_vehicle addEventHandler ["GetOut",{if (_this select 2 == player) then {[] call cTab_fnc_close}}]
	];
};
...
if (_displayName in ["cTab_TAD_dsp","cTab_TAD_dlg"]) then {
	cTabIfOpen set [6,
		addMissionEventHandler ["Draw3D",{
			...
			(_display displayCtrl IDC_CTAB_OSD_DIR_DEGREE) ctrlSetText format ["%1°",[direction _veh,3] call cTab_fnc_formatNumber];
			
			// update current elevation (ASL) on TAD
			(_display displayCtrl IDC_CTAB_OSD_ELEVATION) ctrlSetText format ["%1m",[round (_playerPos select 2),4] call cTab_fnc_formatNumber];
		}]
	];
} else {
	cTabIfOpen set [6,
		addMissionEventHandler ["Draw3D",{
			...
			// update current heading
			(_display displayCtrl IDC_CTAB_OSD_DIR_DEGREE) ctrlSetText format ["%1°",[_heading,3] call cTab_fnc_formatNumber];
