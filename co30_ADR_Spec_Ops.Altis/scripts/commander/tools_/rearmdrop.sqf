...
_cargo = createVehicle ["B_Slingload_01_Ammo_F", position _parachute, [], 0, 'NONE'];
_cargo attachTo [_parachute, [0, 0, -0.4]];
_cargo allowDamage false;
_try = true;
while {_try} do {
    _cargo setAmmoCargo 0.000000032562; // Set cargo ammo to 32 562 instead of 10^12
    if (getAmmoCargo _cargo != 0) then { // Due to very low values setAmmoCargo rounds to 0 on the first try
        _try = false;
    };
};
