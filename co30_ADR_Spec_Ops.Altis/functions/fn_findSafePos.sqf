/*
Author: ToxaBes
Description: a bit custom implementation of BIS_fnc_FindSafePos function
Parameter(s):
...
_this select 8: (optional) default position on land or water (Array):
Returns: Coordinate array.
*/
scopeName "main";
private ["_pos", "_minDist", "_maxDist", "_objDist", "_waterMode", "_maxGradient", "_shoreMode", "_defaultPos", "_blacklist", "_newPos", "_posX", "_posY", "_attempts", "_newX", "_newY", "_testPos", "_result"];
_pos = _this select 0;
	...
	if (count _blacklist > 0) then {
	    if (!([_testPos, _blacklist] call BIS_fnc_isPosBlacklisted)) then {
			if ((_pos distance _testPos) >= _minDist) then {
				if (!((count (_testPos isFlatEmpty [_objDist, -1, _maxGradient, _objDist max 5, _waterMode, _shoreMode])) == 0)) then {
					_newPos = _testPos;
					breakTo "main";
				};
			};
