...
*/
private ["_target", "_targets", "_prevPosition", "_prevPrevPosition", "_targetCoords", "_disatnce1", "_disatnce2"];
...
_prevPosition = missionNamespace getVariable "PREV_POSITION";
_prevPrevPosition = missionNamespace getVariable "PREV_PREV_POSITION";
if (isNil ("_prevPosition")) then {	
	missionNamespace setVariable ["PREV_POSITION", _target];
} else {
    _prevCoords = _prevPosition select 1;
    if (isNil ("_prevPrevPosition")) then {
    	_targetCoords = _target select 1;
    	_disatnce1 = _targetCoords distance2D _prevCoords;
        while {_disatnce1 < 2000 || _disatnce1 > 8000} do {
            _target = _targets call BIS_fnc_selectRandom;
    	    _targetCoords = _target select 1;
    	    _disatnce1 = _targetCoords distance2D _prevCoords;
        };   
    	missionNamespace setVariable ["PREV_POSITION", _target];
    	missionNamespace setVariable ["PREV_PREV_POSITION", _prevPosition];
    } else {
        _prevPrevCoords = _prevPrevPosition select 1;
    	_targetCoords = _target select 1;
    	_disatnce1 = _targetCoords distance2D _prevCoords;
    	_disatnce2 = _targetCoords distance2D _prevPrevCoords;
        while {_disatnce1 < 2000 || _disatnce1 > 7000 || _disatnce2 < 3000} do {
            _target = _targets call BIS_fnc_selectRandom;
    	    _targetCoords = _target select 1;
    	    _disatnce1 = _targetCoords distance2D _prevCoords;
        };   
