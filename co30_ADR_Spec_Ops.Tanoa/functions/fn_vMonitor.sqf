...
_maxScore = param [5, 9999];  // max allowed score for upgrade
_t = typeOf _v;              // type
...
// MONITOR LOOP
while {true} do {
    if (BLUFOR_BASE_SCORE < _score || BLUFOR_BASE_SCORE > _maxScore) then {
        if (alive _v) then {
        ...
        if (!alive _v) then {
            ...
            waitUntil {sleep 10; (BLUFOR_BASE_SCORE >= _score && BLUFOR_BASE_SCORE <= _maxScore)};
            if (!isNull _v) then {
                deleteVehicle _v;
            };
