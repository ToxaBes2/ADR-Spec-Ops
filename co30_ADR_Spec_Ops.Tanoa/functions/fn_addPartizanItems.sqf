...
if (_level > 10) then {
    switch (typeOf _player) do { 
        ...
        case "I_C_Soldier_Para_8_F": {
            _weapons = [
                ["srifle_DMR_06_olive_F", ["20Rnd_762x51_Mag"]],
                ["arifle_AKM_F", ["30Rnd_762x39_Mag_F"]]
            ];
        }; 
        case "I_C_Soldier_Para_3_F": {
            _weapons = [
...
_player addItem "FirstAidKit";
_player addItem "FirstAidKit";
if (_player getUnitTrait "medic" || typeOf _player in ["I_C_Soldier_Para_3_F"]) then {
    _player addItem "Medikit";
};
if (_player getUnitTrait "engineer" || typeOf _player in ["I_C_Soldier_Para_8_F"]) then {
