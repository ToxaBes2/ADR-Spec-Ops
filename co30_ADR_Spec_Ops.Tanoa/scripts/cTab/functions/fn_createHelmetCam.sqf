...
_visualModeHCAM = ["cTab_Tablet_dlg","visualModeHCAM"] call cTab_fnc_getSettings;
...
if (!isNil "cTabHcams") then {
	...
	if (_oldHost isEqualTo _newHost) then {
		_renderTarget setPiPEffect [_visualModeHCAM];
...
_renderTarget setPiPEffect [_visualModeHCAM];
cTabHcams = [_cam,_target,_newHost,_renderTarget];
true