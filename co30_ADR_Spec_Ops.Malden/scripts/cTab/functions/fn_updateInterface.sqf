...

{
	call {
		...
		if (_x select 0 == "nightMode") exitWith {
			...
			_background = call {				
				if (_displayName in ["cTab_Tablet_dlg"]) exitWith {
					if (_nightMode) then {"scripts\cTab\img\tablet_background_night_ca.paa"} else {"scripts\cTab\img\tablet_background_ca.paa"};
				};
				""
			};
			if (_background != "") then {
		...
		if (_x select 0 == "mode") exitWith {
			...
			_displayItems = call {				
				if (_displayName == "cTab_Tablet_dlg") exitWith {
					...
					IDC_CTAB_CTABUAV_FULL,
					IDC_CTAB_CTABUAV_FULL2,
					IDC_CTAB_GROUP_SUPPORT,
					IDC_CTAB_SUPPORT,
					IDC_CTAB_SUPPORT_TOPO,
					IDC_CTAB_BALLISTIC,
					IDC_CTAB_GROUP,
					IDC_CTAB_GROUP_GROUP
					]
				};				
			...
			if !(_displayItems isEqualTo []) then {
				...
				call {
					...
					if (_mode == "UAV") exitWith {
						...
						_btnActCtrl ctrlSetTooltip "На весь экран";
						_settings pushBack ["uavListUpdate",true];
						if (!_interfaceInit) then {
							_settings pushBack ["uavCam",[_displayName,"uavCam"] call cTab_fnc_getSettings];
						};
					};
                    // Fullscreen UAV cam 1                    
                    if (_mode == "UAV_FULL") exitWith {
						_displayItemsToShow = [IDC_CTAB_CTABUAV_FULL];
						_data = [_displayName,"uavCam"] call cTab_fnc_getSettings;
						_btnActCtrl ctrlSetTooltip "Оконный режим";
                        [_data,[[0,"rendertarget14"]]] spawn cTab_fnc_createUavCam;
					};
					// Fullscreen UAV cam 2                    
                    if (_mode == "UAV_FULL2") exitWith {
						_displayItemsToShow = [IDC_CTAB_CTABUAV_FULL2];
						_data = [_displayName,"uavCam"] call cTab_fnc_getSettings;
						_btnActCtrl ctrlSetTooltip "Оконный режим";
                        [_data,[[1,"rendertarget15"]]] spawn cTab_fnc_createUavCam;
					};
					// ---------- HELMET CAM -----------
					...
					if (_mode == "SUPPORT") exitWith {
						_displayItemsToShow = [IDC_CTAB_SUPPORT];
						_btnActCtrl ctrlSetTooltip "";
					};
                    // ---------- BALLISTIC -----------
					if (_mode == "BALLISTIC") exitWith {
						_displayItemsToShow = [IDC_CTAB_BALLISTIC,IDC_CTAB_GROUP_BALLISTIC];
						_btnActCtrl ctrlSetTooltip "";
					};
					// ---------- GROUP -----------
					if (_mode == "GROUP") exitWith {
						_displayItemsToShow = [IDC_CTAB_GROUP,IDC_CTAB_GROUP_GROUP];
						_btnActCtrl ctrlSetTooltip "";
					};
				};
			...
			if (_mode == "BFT" && !_isDialog) then {
				...
				if (!isNull _osdCtrl) then {
					...
					_mapScaleTxt = if (_mapScaleKm > 1) then {
						...
						[_mapScaleKm / 2,0,1] call cTab_fnc_formatNumber
					};
		...
		if (_x select 0 == "mapWorldPos") exitWith {
			if (_mode == "BFT" || _mode == "SUPPORT" || _mode == "BALLISTIC" || _mode == "GROUP") then {
				if (_isDialog) then {
			...
			if (_mode == "UAV") then {
				...
				// Populate list of UAVs
				{
					if (!(crew _x isEqualTo [])) then {
						_index = _uavListCtrl lbAdd format ["%1:%2 (%3)",groupId group _x,[_x] call cTab_fnc_getGroupIndex,getText (configfile >> "cfgVehicles" >> typeOf _x >> "displayname")];
		...
		if (_x select 0 == "hCamListUpdate") exitWith {
			if (_mode == "HCAM") then {
				...
				{
					_index = _hcamListCtrl lbAdd format ["%1:%2 (%3)",groupId group _x,[_x] call cTab_fnc_getGroupIndex,name _x];
...
if ((!isNil "_targetMapScale") || (!isNil "_targetMapWorldPos")) then {
	...
	if (_mode == "SUPPORT") then {
		_targetMapCtrl = _display displayCtrl IDC_CTAB_SUPPORT;
		if !(ctrlShown _targetMapCtrl) then {
            _targetMapCtrl = _display displayCtrl IDC_CTAB_SUPPORT_TOPO;
		};
	};
	if (_mode == "BALLISTIC") then {
		_grpCtrl = _display displayCtrl IDC_CTAB_GROUP_BALLISTIC;
        _targetMapCtrl = _grpCtrl controlsGroupCtrl IDC_CTAB_BALLISTIC_MAP;
	};
	if (_mode == "GROUP") then {
		_grpCtrl = _display displayCtrl IDC_CTAB_GROUP_GROUP;
        _targetMapCtrl = _grpCtrl controlsGroupCtrl IDC_CTAB_GROUP_MAP;
	};
...
if (!isNull _loadingCtrl) then {
	...
	if (_interfaceInit && _isDialog) then {
		...
		[{
			[_this select 1] call cTab_fnc_removePerFrameHandler;
			setMousePosition (_this select 0);
		},0,_mousePos] call cTab_fnc_addPerFrameHandler;
	};
